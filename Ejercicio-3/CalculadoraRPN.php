...
<head>
    ...
    <link type="text/css" href="CalculadoraRPN2.css" rel="stylesheet">
</head>
...
<body>
    ...
    $_SESSION['contador'] = $miCalculadora->stack;
    if (isset($_SESSION['current'])) {
        $_SESSION['current'] .= $miCalculadora->current;
    } else {
        $_SESSION['current'] = $miCalculadora->current;
    }
    ...
    $contador = $stack->len();
    echo "<textarea id='pila' rows='6' cols='35' readonly>";

    foreach (array_reverse($stack->stack()) as $element) {

        echo  $contador . ":   " .  $element . "\n";
        $contador = $contador - 1;
    }
    echo "</textarea>";
    echo "</div>";
    ...
    <div class='fila'>
        <button type='submit' class='gris'></button>
    ...
    <div class='fila'>
        <button type='submit' id='CE' name='CE'>CE</button>
